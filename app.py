...
def plot_basis_grid(M, lim=4.0, show_grid=True, show_para=True, show_inv=False):
    """기준점 (1,0), (0,1)이 어디로 가는지 + 격자 전체의 변형을 보여준다"""
    ...
    for tip in [(1, 0), (0, 1)]:                     # 원래 기준점 (1,0), (0,1) (점선)
...
with tabs[0]:
    ...
    st.caption("행렬 A는 기준점 (1,0)과 (0,1)을 각각 u=(a,c), v=(b,d) 로 보냅니다. "
               "그 둘이 만드는 평행사변형의 넓이가 곧 행렬식입니다.")

    # ⚠️ 슬라이더에 key를 주면 위젯 상태가 value= 보다 우선한다.
    #    따라서 프리셋은 session_state의 '슬라이더 키'를 직접 바꿔야 반영된다.
    ...
    with left:
        ...
        st.button("🎬 붕괴 시연 준비", use_container_width=True, on_click=set_bg,
                  args=(1., 1., 0., 1.),
                  help="누른 뒤 c 슬라이더를 0 → 1.0 으로 천천히 밀면 격자가 무너집니다")
    ...
    # 지금 상태에서 어떤 값을 바꾸면 det = 0 이 되는지 안내
    if abs(dM) > 1e-9:
        hints = []
        ...
        if hints:
            st.caption("🎯 지금 상태에서 " + " 또는 ".join(hints)
                       + " 맞추면 det = 0 이 되어 격자가 무너집니다.")

    if abs(dM) < 1e-9:
        st.error("**det = 0** — 두 화살표가 일직선 위에 놓였습니다. "
...
> 📌 이 자료는 행렬의 **1열 (a,c)** 가 u, **2열 (b,d)** 가 v 입니다.
> 책자 2~3쪽과 같은 방식이니 수학 시간에 배운 그대로예요.
>
> 📚 참고: 점 (1,0), (0,1)처럼 **기준이 되는 두 점**을 대학에서는
> **기저벡터(basis vector)** 라고 부릅니다. 지금은 몰라도 괜찮아요 —
> 인터넷에서 자료를 찾다 이 말이 나오면 "아, 그 두 점이구나" 하면 됩니다.
        """)
...
with tabs[5]:
    ...
    # 탭0과 같은 이유로, 프리셋은 위젯 키를 직접 바꿔야 반영된다.
    for _r in range(3):
        for _c in range(3):
            st.session_state.setdefault(f"k_{_r}_{_c}",
                                        float(KP["엣지 - 세로"][_r][_c]))

    def set_kernel(name):
        for _r in range(3):
            for _c in range(3):
                st.session_state[f"k_{_r}_{_c}"] = float(KP[name][_r][_c])

    c1, c2 = st.columns([1, 1.7])
    with c1:
        pick = st.selectbox("프리셋", list(KP.keys()), index=3)
        st.button("프리셋 적용", use_container_width=True,
                  on_click=set_kernel, args=(pick,))
        ...
        for r in range(3):
            ...
            for cc in range(3):
                K[r, cc] = cols[cc].number_input(
                    f"k{r}{cc}", step=0.5, key=f"k_{r}_{cc}",
